...
const Repos = (): JSX.Element => {
	...
	const handleRepoClick = (item: any) => {
		const {
			owner: { login },
			name,
		} = item
		Router.navigateTo({
			url: `/repo/detail?owner=${login}&name=${name}`,
		})
	}
					...
					return (
						...
						>
							<View className='repo-item-header'>
								<View className='repo-item-title'>{item.name}</View>
								<View className='repo-item-starrgazer'>
									⭐️ {item.stargazers_count}
								</View>
							</View>
							<View className='repo-item-desc'>{item.description}</View>
							<View className='repo-item-bottom'>
								<View className='repo-item-language'>{item.language}</View>
